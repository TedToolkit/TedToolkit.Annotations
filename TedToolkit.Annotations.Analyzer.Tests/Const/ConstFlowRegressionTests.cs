...
{
    /// <summary>
    /// 验证 Explicit.Const 不会清除来源参数已有的 Const 契约。
    /// </summary>
    [Test]
    public async Task Should_preserve_source_contract_for_explicit_const_local()
    {
        var diagnostics = await ConstAnalyzerTestHelper.AnalyzeAsync("""
            ...
            sealed class Sample
            {
                ...
                {
                    var local = Explicit.Const(source, ConstDepth.NONE);
                    local.Value = 1;
                }
            }
            """);
    ...
    /// <summary>
    /// 验证 Explicit.Const 局部变量重新赋值后仍保留自身契约。
    /// </summary>
    [Test]
    public async Task Should_preserve_local_contract_after_reassignment()
    {
        var diagnostics = await ConstAnalyzerTestHelper.AnalyzeAsync("""
            ...
            sealed class Sample
            {
                ...
                {
                    var local = Explicit.Const(first, ConstDepth.DEPTH1);
                    local = second;
                    local.Value = 1;
                }
            }
